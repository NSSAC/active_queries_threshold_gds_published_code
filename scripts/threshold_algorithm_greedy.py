#!/usr/bin/env python
# tags: code python thresholdAlgorithm threshold imported
...
-----
Operation: This is a library with a simple example. It will be imported by
multiple scripts for each experiment direction.
"""
...
def successorStandardThreshold(G,T,q,q_):
   for v in G.nodes():
      hammWt=hammingWeight(itertools.chain([v],G.neighbors(v)),q) 
      if hammWt>=T[v]:
         q_[v]=1
      else:
         q_[v]=0
...
def greedyDistanceTwo(G,T,successor,Gsqr=False):
   ...
   while True:
      ...
      while sum(nodesToConsider):
         ...
         for v in itertools.chain([vmax],Gsqr.neighbors(vmax)):
      ...
      totalNodesToConsider=np.sum(tHigh!=tLow)
   ...
   if np.array_equal(T,tHigh):
      ...
      return True,numQueries,tHigh
   else:
      logging.debug("They don't match!")
      return False,numQueries,tHigh
...
def exampleCompleteGraphSameThresholds():
   ...
   for threshold in range(numNodes):
      T.fill(threshold)
